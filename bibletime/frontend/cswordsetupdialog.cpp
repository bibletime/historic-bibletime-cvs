...
CInstallSourcesMgrDialog::InstallSourceItem::InstallSourceItem( QListView* parent ) :
	QListViewItem(parent, QString::null)
{

}

CInstallSourcesMgrDialog::InstallSourceItem::InstallSourceItem( QListView* parent, sword::InstallSource is ) : QListViewItem(parent, QString::null)
{
...
void CInstallSourcesMgrDialog::initLocalSourcesPage() {
	...
	m_localSourcesList = new QListView( m_localSourcesPage );
...
// 	m_localSourcesList->setFullWidth(true);
	...
	for (QStringList::iterator it = sources.begin(); it != sources.end(); ++it) {
		...
		/*QListViewItem* i = */new QListViewItem( m_localSourcesList, *it );
		//i->setText(0, *it);
	}
...
void CInstallSourcesMgrDialog::slot_localAddSource() {
  ...
  if (url.isValid()) {
		new QListViewItem(m_localSourcesList, url.path());
  }
}
...
void CInstallSourcesMgrDialog::initRemoteSourcesPage() {
	...
	m_remoteSourcesList = new QListView( m_remoteSourcesPage );
