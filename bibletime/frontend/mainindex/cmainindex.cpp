...
bool CMainIndex::acceptDrag( QDropEvent* event ) const {
    ...
    //isMovable is a hack so we can drop between bookmarks
    return i ? (i->acceptDrop(event) || i->isMovable()) : false;
    //   return i ? i->acceptDrop(event) : false;
}
...
void CMainIndex::dropped( QDropEvent* e, QListViewItem* parent, QListViewItem* after) {
    ...
    CItemBase* dropItem = 0;
    if (onItem && onItem->acceptDrop(e)) {
        dropItem = onItem;
    } else if (parentItem && parentItem->acceptDrop(e)) {
        dropItem = parentItem;
    }

    if (dropItem) {
        dropItem->dropped(e,after);
