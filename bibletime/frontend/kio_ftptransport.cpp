...
char KIO_FTPTransport::getURL(const char *destPath, const char *sourceURL) {
	...
	KApplication::kApplication()->enter_loop(); //enters the event loop; slotCopyResult will make it terminate again

	statusReporter->statusUpdate(m_totalSize, m_totalSize); //completed

	if (!m_copyResults.contains(progressID) || (m_copyResults[progressID] > 0)) return 1; //Error
	return 0;
}
...
void KIO_FTPTransport::slotCopyProgress(KIO::Job* job, KIO::filesize_t processedSize) {
	...
	if (term && job){ //the user cancelled the operation in the meanwhile
