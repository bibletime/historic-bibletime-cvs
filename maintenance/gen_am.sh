...
export FOREIGN_PO_LANGUAGES="af bg cs de en_GB es fi fr it ko nl nn_NO pl pt_br ro ru sk ua zh_CN zh_TW"
# The following languages are currently not complete enough:
# hu no

export FOREIGN_DOC_LANGUAGES="bg cs de fi fr it ko nl pt-br ru"
...
( # output to Makefile.am
	...
	for PART in $DOC_PARTS; do
		if test "$PART" = "howto"; then
			echo -n "\$(HOWTODIR) " #can be turned off by configure because of license problems
		else
			echo -n "$PART "
		fi
	done
...
for PART in $DOC_PARTS; do
	...
	#####################################
	#create $PART/$SECTION/Makefile.am
	#####################################
	...
	for SECTION in unicode html; do
		...
		( # output to Makefile.am
			...
			if test $SECTION = "html"; then
				...
				FILES=`ls -d $I2/*.docbook 2>/dev/null`;
			fi;
...
for I1 in $FOREIGN_DOC_LANGUAGES; do
	...
	for PART in $DOC_PARTS; do
		...
		if test -d ../../bibletime-i18n/docs/$I1/$PART; then
			...
			#####################################
			#create bibletime-i18n/docs/??/$PART/$SECTION/Makefile.am
			#####################################
			for SECTION in unicode; do #html files are no longer installed
