...
// 				fprintf( stderr,"(BibleInstaller %s) Debug: %s\n",VERSION, msg );
// 			break;
// 		case QtWarningMsg:
// 				fprintf( stderr,"(BibleInstaller %s) WARNING: %s\n",VERSION, msg );
// 			break;
// 		case QtFatalMsg:
// 			fprintf( stderr,"(BibleInstaller %s) _FATAL_: %s\nPlease contact info@bibletime.de and report this bug!",VERSION, msg );
...
// 		fprintf(stderr, "*** BibleInstaller got signal %d (Exiting)\n", sigId);
...
// 		setSignalHandler(SIG_DFL);
// 		fprintf(stderr, "*** BibleInstaller got signal %d (Crashing). Trying to save settings.\n", sigId);
// 		if (bibletime) {
// 			bibletime->saveSettings();
// 			fprintf(stderr, "*** Saving seemed to be succesful. If restoring does not work on next startup \
// please use the option --ignore-startprofile\n");
