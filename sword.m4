...
AC_FIND_FILE($SEARCH_LIBS, $ac_sword_library_dirs, ac_sword_libdir)

if test "$ac_sword_libdir" = "NO"; then
  AC_MSG_ERROR(SWORD library not found. Try to use configure with --with-sword-dir=/your/SWORD/path!);
fi

if test "$ac_static_sword" = "YES"; then
	LIB_SWORD="$ac_sword_libdir/libsword.a";
else
	LIB_SWORD="-lsword";
fi
...
if test "$ac_static_sword" = "YES"; then
	MESSAGE="static library $ac_sword_libdir/libsword.a";
else
	MESSAGE="$ac_sword_libdir";
fi
...
AC_FIND_FILE(swmgr.h, $ac_sword_include_dirs, ac_sword_incdir)
if test "$ac_sword_incdir" = "NO"; then
	AC_MSG_ERROR([The Sword include file files were not found.
Please try to use configure with --with-sword-dir=/your/SWORD/path !
])
fi

SWORD_INCLUDES="-I$ac_sword_incdir"
AC_SUBST(SWORD_INCLUDES)
all_includes="$all_includes -I$ac_sword_incdir"

AC_MSG_RESULT([$ac_sword_incdir])
...
CXXFLAGS="$CXXFLAGS -I$"
LDFLAGS="$LDFLAGS -L$ac_sword_libdir"
LIBS="$LIB_SWORD -lz"
LD_LIBRARY_PATH="$ac_sword_libdir"
...
if AC_TRY_EVAL(ac_link) && test -s conftest; then
	if test -x conftest; then
		eval ac_installed_sword_version=`./conftest 2>&5`
	fi
else
...
AC_MSG_RESULT([$ac_installed_sword_version])
])
